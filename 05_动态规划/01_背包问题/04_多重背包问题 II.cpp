...
备注：为什么完全背包问题没有f[i-1][j-(k+1)v + kw]，而多重背包有吗？答案是完全背包是没有显示第i个物品的个数，也就是没有s[i]，它可以一直装到背包满为止，
...
通过第一组可以表示处理 0-1，把第一组表示的范围加上第二组，得到2-3，那么把它们拼接就可以得到0-3
然后在加上第三组，得到4-7，与上面的拼接得到0-7
以此类推可以得到0-s
按照这种方式，可以把 n * m * s 优化成 n * m * log(s)，也就是 1000 * 2000 * log(2000) = 1e7，是在范围之内的
然后把这n个物品差分之后可以得到 1000 * log2000 = 11000-12000个物品，如果用二维数组会开12010 * 2010大小的数组，太大了
